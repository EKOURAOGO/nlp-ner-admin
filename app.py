...
.ent-token { padding: 3px 7px; border-radius: 5px; font-size: 0.9rem; font-weight: 500; margin: 0 1px; }
.ent-label { font-size: 0.58rem; font-weight: 700; vertical-align: super; margin-left: 3px; }
.inference-box { background: #111318; border: 1px solid #1C1F28; border-radius: 10px;
    padding: 1.4rem 1.6rem; margin: 1rem 0; line-height: 2.4; font-size: 0.95rem; color: #CBD5E1; }
.ent-chip { display: inline-flex; align-items: center; gap: 8px; background: #111318;
    border: 1px solid #1C1F28; border-radius: 8px; padding: 0.6rem 1rem; margin: 0.3rem; }
.ent-chip-lbl { font-size: 0.65rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.1em; }
.ent-chip-val { font-size: 0.88rem; font-weight: 600; color: #F1F5F9; }
...
[aria-selected="true"][data-baseweb="tab"] {
    color: #E2E8F0 !important; border-bottom: 2px solid #10B981 !important; background: transparent !important; }
.stTextArea textarea { background: #111318 !important; color: #CBD5E1 !important;
    border: 1px solid #2D3142 !important; border-radius: 8px !important; font-size: 0.9rem !important; }
</style>
...
def make_fig(title=""):
    """Crée un layout Plotly de base sans conflit d'axes."""
    layout = dict(
    ...
    if title:
        layout["title"] = dict(text=title, font=dict(size=12, color="#94A3B8"))
    return layout
...
def highlight(text, entities):
    ...
    for s, e, label in sorted(entities, key=lambda x: x[0]):
        ...
        result.append(
            ...
            text[s:e] +
            f'<span class="ent-label" style="color:{st_["fg"]}">{label}</span></span>'
        )
        prev = e
...
with st.sidebar:
    ...
    for label, st_ in ENT_STYLES.items():
        st.markdown(
            f'<span style="background:{st_["bg"]};color:{st_["fg"]};border:1px solid {st_["border"]};'
            f'padding:2px 8px;border-radius:5px;font-size:0.75rem;font-weight:600">{label}</span>',
            unsafe_allow_html=True
        )
...
<div class="app-header">
  ...
  <div class="app-subtitle">spaCy blank("fr") · 5 types d'entités · {int(len(dataset)*0.8)} exemples d'entraînement</div>
</div>
<div class="kpi-grid">
  <div class="kpi-card"><div class="kpi-label">Train</div><div class="kpi-value">{int(len(dataset)*0.8)}</div><div class="kpi-sub">80% du dataset</div></div>
  <div class="kpi-card"><div class="kpi-label">Test</div><div class="kpi-value">{int(len(dataset)*0.2)}</div><div class="kpi-sub">20% du dataset</div></div>
  <div class="kpi-card"><div class="kpi-label">Macro F1</div><div class="kpi-value">{macro.get("f1",0):.3f}</div><div class="kpi-sub">5 entités</div></div>
  <div class="kpi-card"><div class="kpi-label">Précision</div><div class="kpi-value">{macro.get("precision",0):.3f}</div><div class="kpi-sub">Macro moyenne</div></div>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    rows = [{"Entité": l,
             "Précision": metrics.get(l, {}).get("precision", 0),
             "Rappel":    metrics.get(l, {}).get("recall", 0),
             "F1":        metrics.get(l, {}).get("f1", 0),
             "Support":   metrics.get(l, {}).get("support", 0)}
    ...
    with cl:
        fig = go.Figure()
        for metric, color in [("F1","#10B981"), ("Précision","#6366F1"), ("Rappel","#F59E0B")]:
            fig.add_trace(go.Bar(
                name=metric, x=df_m["Entité"], y=df_m[metric],
                marker=dict(color=color, opacity=0.85, cornerradius=3)
            ))
        layout = make_fig("F1 · Précision · Rappel")
        layout.update({"barmode": "group", "height": 300,
                        "yaxis": dict(range=[0,1.15], gridcolor="#1C1F28", linecolor="#1C1F28"),
    ...
    with cr:
        ...
        vals = [metrics.get(l,{}).get("f1",0) for l in ENTITY_LABELS]
        ...
        fig_r.update_layout(
            ...
            title=dict(text="Vue radar F1", font=dict(size=12, color="#94A3B8"))
        )
        st.plotly_chart(fig_r, use_container_width=True)

    st.dataframe(
        df_m.style.format({"Précision":"{:.3f}","Rappel":"{:.3f}","F1":"{:.3f}"}),
...
with tab2:
    ...
    with col_ex:
        ...
        chosen = None
        for ex in EXAMPLES:
            if st.button(ex[:38]+"…", key=f"ex_{ex[:8]}", use_container_width=True):
                chosen = ex

    with col_inp:
        default = chosen if chosen else EXAMPLES[0]
        user_input = st.text_area("Texte :", value=default, height=110,
    ...
    if user_input:
        ...
        st.markdown(
            f'<div class="inference-box">{highlight(user_input, ents)}</div>',
            unsafe_allow_html=True
        )
        if ents:
            ...
            for s, e, label in ents:
                st_ = ENT_STYLES.get(label, {"bg":"#1C1F28","fg":"#E2E8F0","border":"#2D3142"})
                chips += (
                    f'<span class="ent-chip" style="border-color:{st_["border"]}">' +
                    f'<span class="ent-chip-lbl" style="color:{st_["fg"]}">{label}</span>' +
                    f'<span class="ent-chip-val">{user_input[s:e]}</span></span>'
                )
            st.markdown(f'<div style="display:flex;flex-wrap:wrap">{chips}</div>',
                        unsafe_allow_html=True)
        else:
            st.markdown('<p style="color:#475569;font-size:0.85rem;margin-top:0.5rem">Aucune entité détectée.</p>',
                        unsafe_allow_html=True)
...
with tab3:
    ...
    fig_l.add_trace(go.Scatter(
        x=df_h["iter"], y=df_h["loss"], mode="lines",
        line=dict(color="#F59E0B", width=2),
        fill="tozeroy", fillcolor="rgba(245,158,11,0.07)"
    ))
    layout_l = make_fig("Convergence NER")
    layout_l.update({"height": 280, "xaxis": dict(title="Itération", gridcolor="#1C1F28", linecolor="#1C1F28"),
                     "yaxis": dict(title="Loss NER", gridcolor="#1C1F28", linecolor="#1C1F28")})
    fig_l.update_layout(**layout_l)
    st.plotly_chart(fig_l, use_container_width=True)
    final_loss = df_h["loss"].iloc[-1]
    st.markdown(
        f'<p style="font-size:0.8rem;color:#475569">Loss finale : ' +
        f'<strong style="color:#F59E0B">{final_loss:.4f}</strong> après {n_iter} itérations.</p>',
        unsafe_allow_html=True
    )
